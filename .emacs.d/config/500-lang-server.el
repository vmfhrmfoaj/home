        ...
        (dolist (diagnostic diagnostics)
          (let ((messages (-some->> diagnostic
                ...
                (face (let ((level (lsp-diagnostic-severity diagnostic)))
                        (cond
                         ((eq 4 level) `(:inherit compilation-info :weight normal))
                         ((eq 3 level) `(:inherit compilation-info :weight normal))
                         ((eq 2 level) `(:inherit compilation-warning :weight normal))
                         ((eq 1 level) `(:inherit compilation-error :weight normal)))))
